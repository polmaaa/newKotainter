...
export default function CrmMutasiUnit({ user, apiBaseUrl, showToast, isPostgres = true, menuName = 'Mutasi Unit CRM' }) {
  ...
  // Form inputs
  const [newWorkingUnit, setNewWorkingUnit] = useState('');
  const [noTiket, setNoTiket] = useState('');
  const [saving, setSaving] = useState(false);

  const dbPrefix = 'api/Crm_mutasi_unit';
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (response.ok && result.status === 'success') {
        ...
        setNewWorkingUnit(result.data.working_unit || '');
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    if (!userData) return;
    if (!newWorkingUnit.trim()) {
      showToast('Unit Kerja Baru wajib diisi!', 'warning');
      return;
    }
    if (!noTiket.trim()) {
    ...
    setSaving(true);
    try {
      const response = await fetch(`${apiBaseUrl}/${dbPrefix}/save_mutasi`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          working_unit: newWorkingUnit.trim(),
          no_tiket: noTiket.trim()
        })
      });
      const result = await response.json();
      if (response.ok && result.status === 'success') {
        showToast(result.message || 'Mutasi unit kerja user CRM berhasil disimpan!', 'success');
        setUserData(prev => ({ ...prev, working_unit: newWorkingUnit.trim() }));
        setNoTiket('');
      } else {
        showToast(result.message || 'Gagal memperbarui unit kerja.', 'error');
      }
    } catch (err) {
      console.error(err);
      showToast('Gagal terhubung ke API backend.', 'error');
  ...
  return (
    <div className="crm-mutasi-container" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      {/* Title block */}
      <div className="title-area" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h2 style={{ margin: 0, color: 'var(--text-main)', fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <i className="pi pi-directions" style={{ color: '#0f766e' }}></i> {menuName}
          </h2>
          <p style={{ margin: '4px 0 0 0', color: 'var(--text-muted)', fontSize: '0.9rem' }}>
            Mengubah dan memutasi kode unit kerja (working_unit) user CRM pada PostgreSQL.
          </p>
        </div>
        <span style={{ 
          padding: '6px 12px', 
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '24px' }}>
          ...
          <div className="content-card" style={{ padding: '20px', borderRadius: '12px', backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-light)', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', display: 'block' }}>Unit Kerja Saat Ini</span>
                <span style={{ fontWeight: 600, color: '#0f766e', fontSize: '1.05rem' }}>{userData.working_unit || 'TIDAK ADA'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="content-card" style={{ padding: '20px', borderRadius: '12px', backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-light)', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
            <h3 style={{ margin: '0 0 16px 0', fontSize: '1rem', color: '#0f766e', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <i className="pi pi-pencil"></i> Proses Mutasi Kerja
            </h3>
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-main)' }}>Unit Kerja Baru</label>
                <input
                  type="text"
                  placeholder="Masukkan Kode Unit Baru"
                  value={newWorkingUnit}
                  onChange={(e) => setNewWorkingUnit(e.target.value)}
                  style={{
                    padding: '10px 14px',
                    borderRadius: '8px',
                    border: '1px solid var(--border-color)',
              ...
              >
                {saving ? (
                  ...
                  <>
                    <i className="pi pi-save"></i> Simpan Mutasi
                  </>
                )}
              </button>
            </form>
